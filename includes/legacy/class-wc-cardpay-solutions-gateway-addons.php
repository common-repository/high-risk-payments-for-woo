<?php
/**
 * Class WC_Cardpay_Solutions_Gateway_Addons legacy file.
 *
...
class WC_Cardpay_Solutions_Gateway_Addons extends WC_Cardpay_Solutions_Gateway {
	...
	protected function save_subscription_meta( $order_id, $card ) {
		$order = wc_get_order( $order_id );
		$order->update_meta_data( '_cardpay_token', $card['token'] );
		$order->save();
		...
		foreach ( wcs_get_subscriptions_for_order( $order_id ) as $subscription ) {
			$subscription->update_meta_data( '_cardpay_token', $card['token'] );
			$subscription->save();
		}
	}
	...
	protected function process_pre_order( $order_id ) {
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order_id ) ) {
			...
			try {
				...
				$order->update_meta_data( '_cardpay_token', $card_meta['token'] );
				$order->save();
	...
	public function process_subscription_payment( $order, $amount = 0 ) {
		$card = array(
			'token' => $order->get_meta( '_cardpay_token', true ),
		);
